...
public class FileStorage : IFileStorage
{
    ...
    public FileStorage(IOptions<AzureSetting> azureOption, IStringLocalizer<Errors> localizer)
    {
    ...
    public async Task<string> SaveFileAsync(byte[] content, string fileName, string containerName, string fileNameOriginal, string mimeTypeEnviado)
    {
        ...
        if (!validExts.Contains(ext))
            throw new InvalidOperationException(_localizer[nameof(Errors.Errors_InvalidFileExtension)]);
        ...
        if (mimeEsperado?.Trim().ToLowerInvariant() != mimeTypeEnviado?.Trim().ToLowerInvariant())
            throw new InvalidOperationException(_localizer[nameof(Errors.Errors_InvalidMimeType)]);
